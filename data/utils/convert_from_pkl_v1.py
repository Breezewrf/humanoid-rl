...
def compute_forward_kinematics(root_rot_wxyz, motion_data, N_bodies, T, robot_name):
    """
    使用MuJoCo直接计算每个body在世界坐标系下的四元数（优先）。
    如果找不到MuJoCo或robot XML，回退到原先的近似叠加方法。
    ...
    if robot_name == "unitree_g1":
        xml_candidates.append(repo_root / "sim2real" / "data" / "robots" / "g1" / "g1_23dof_rubberhand-suitcase.xml")
        xml_candidates.append(repo_root / "sim2real" / "data" / "robots" / "g1" / "g1_23dof_rubberhand.xml")
    # allow user-specified XML via motion_data
    if "xml_path" in motion_data:
        xml_candidates.insert(0, Path(motion_data["xml_path"]))
        ...
        try:
            ...
            try:
                ...
                for t in range(T):
                    ...
                    sim.data.qpos[:qpos.shape[0]] = qpos
                    sim.forward()

                    # mujoco_py: sim.data.xquat (flattened) or sim.data.body_xquat
                    # use sim.data.xquat which is (nbody,4) in many mjcf builds
                    try:
                        xquat = sim.data.xquat.reshape(-1, 4)
                    except Exception:
                        # alternative name
                        xquat = getattr(sim.data, "body_xquat", None)
                        if xquat is None:
                            xquat = np.tile(root_rot_wxyz[t], (N_bodies, 1))
                    nq_body = min(N_bodies, xquat.shape[0])
                    body_quat_w[t, :nq_body, :] = xquat[:nq_body, :]
                    for b in range(nq_body, N_bodies):
                        body_quat_w[t, b] = root_rot_wxyz[t]
                return body_quat_w
            except Exception:
