...
def filter_data(npz_path, json_path, output_path):
    ...
    body_indices_to_remove = [
        body_names.index("right_wrist_pitch_link"),
        body_names.index("right_wrist_yaw_link"),
        body_names.index("left_wrist_pitch_link"),
        body_names.index("left_wrist_yaw_link")
    ]

    joint_indices_to_remove = [
        joint_names.index("waist_roll_joint"),
        joint_names.index("waist_pitch_joint"),
        joint_names.index("right_wrist_pitch_joint"),
        joint_names.index("right_wrist_yaw_joint"),
        joint_names.index("left_wrist_pitch_joint"),
        joint_names.index("left_wrist_yaw_joint")
    ]
    ...
    # Save filtered data
    np.savez(output_path + '/filtered_motion.npz', **filtered_data)
    with open(output_path + '/filtered_meta.json', 'w') as f:
...
if __name__ == "__main__":
    base_path = os.path.expanduser("~/Desktop/workplace/Humanoid/HDMI/data/motion/g1/omomo/sub1_suitcase_011")
    output_path = os.path.expanduser("~/Desktop/workplace/Humanoid/HDMI/data/motion/g1/omomo/sub1_suitcase_011_23dof")
