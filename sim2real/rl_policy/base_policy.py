...
class BasePolicy:
    def __init__(
        ...
        if robot_type == "g1_real" or robot_type == "g1_23dof_real":
        ...
        if robot_config.get("USE_JOYSTICK", False):
            ...
            assert robot_type == "g1_real" or robot_type == "g1_23dof_real", "Joystick control is only supported for g1_real"
    ...
    def setup_policy(self, model_path):
        ...
        if use_residual_action:
            def policy(input_dict):
                ...
                print(f"q_target before: {q_target}, len: {len(q_target)}")
    ...
    def _rl_step_scheduled(self):
        ...
        with Timer(self.perf_dict, "rule_based_control_flow"):
            ...
            self.command_sender.send_command(cmd_q, cmd_dq, cmd_tau)  # send_command(q_target, 0, 0): 只用 位置控制(cmd_q) 运行整个机器人
